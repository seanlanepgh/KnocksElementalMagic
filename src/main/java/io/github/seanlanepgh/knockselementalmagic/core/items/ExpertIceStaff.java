...
import io.github.seanlanepgh.knockselementalmagic.entity.projectile.BlizzBlast;
...
public class ExpertIceStaff extends KnocksBaseItem {

    public ExpertIceStaff(Properties props) {
        super(props);
    }
    ...
    public void releaseUsing(ItemStack stack, Level Level, LivingEntity entityLiving, int timeLeft) {
        ...
        if (entityLiving instanceof Player) {
            ...
            if (!Level.isClientSide) {
                BlizzBlast abstractarrowentity = createArrow(Level, stack, playerentity);
    ...
    public BlizzBlast createArrow(Level level, ItemStack stack, LivingEntity shooter) {
        float j = EnchantmentHelper.getTagEnchantmentLevel(Enchantments.POWER_ARROWS, stack);
        BlizzBlast arrowentity = new BlizzBlast(level, shooter,
                (2.0F + (j * 2.0F)));
        return arrowentity;
    }
