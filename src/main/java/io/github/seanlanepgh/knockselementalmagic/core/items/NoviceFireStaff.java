...
public class NoviceFireStaff extends KnocksBaseItem {

        public NoviceFireStaff(Properties props) {
            super(props);
        }
        @Override
        public void releaseUsing(ItemStack stack,Level Level, LivingEntity entityLiving, int timeLeft) {
                ...
                if (entityLiving instanceof Player) {
                    ...
                    if (!Level.isClientSide) {
                        BlazeStrike abstractarrowentity = createArrow(Level, stack, playerentity);
    ...
    public static void reload(Player user, InteractionHand hand) {
        if (user.getItemInHand(hand).getItem() instanceof NoviceFireStaff) {
            while (!user.isCreative() && user.getItemInHand(hand).getDamageValue() != 0
                    && user.getInventory().countItem(KnocksItems.FIRE_RUNE.get()) > 0) {
                removeAmmo(KnocksItems.FIRE_RUNE.get(), user);
                user.getItemInHand(hand).hurtAndBreak(-2, user, s -> user.broadcastBreakEvent(hand));
                user.getItemInHand(hand).setPopTime(3);
            }
        }
    }
        public BlazeStrike createArrow(Level level, ItemStack stack, LivingEntity shooter) {
            float j = EnchantmentHelper.getTagEnchantmentLevel(Enchantments.POWER_ARROWS, stack);
            BlazeStrike arrowentity = new BlazeStrike(level, shooter,
                    (2.0F + (j * 2.0F)));
            return arrowentity;
        }
