...
public class NoviceEarthStaff extends KnocksBaseItem {

    public NoviceEarthStaff(Properties props) {
        super(props);
    }
    @Override
    public void releaseUsing(ItemStack stack,Level Level, LivingEntity entityLiving, int timeLeft) {
        ...
        if (entityLiving instanceof Player) {
            ...
            if (!Level.isClientSide) {
                QuakeStrike abstractarrowentity = createArrow(Level, stack, playerentity);
...
//                            1.0F / (worldIn.random.nextFloat() * 0.4F + 1.2F) + 0.25F * 0.5F);
    ...
    public static void reload(Player user, InteractionHand hand) {
        if (user.getItemInHand(hand).getItem() instanceof NoviceEarthStaff) {
    ...
    public QuakeStrike createArrow(Level level, ItemStack stack, LivingEntity shooter) {
        float j = EnchantmentHelper.getTagEnchantmentLevel(Enchantments.POWER_ARROWS, stack);
        QuakeStrike arrowentity = new QuakeStrike(level, shooter,
                (2.0F + (j * 2.0F)));
        return arrowentity;
    }
