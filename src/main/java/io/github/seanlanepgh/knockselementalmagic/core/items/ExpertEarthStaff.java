...
import io.github.seanlanepgh.knockselementalmagic.entity.projectile.QuakeBlast;
...
public class ExpertEarthStaff extends KnocksBaseItem {

    public ExpertEarthStaff(Properties props) {
        super(props);
    }
    ...
    public void releaseUsing(ItemStack stack, Level Level, LivingEntity entityLiving, int timeLeft) {
        ...
        if (entityLiving instanceof Player) {
            ...
            if (!Level.isClientSide) {
                QuakeBlast abstractarrowentity = createArrow(Level, stack, playerentity);
    ...
    public static void reload(Player user, InteractionHand hand) {
        if (user.getItemInHand(hand).getItem() instanceof io.github.seanlanepgh.knockselementalmagic.core.items.ApprenticeAirStaff) {
            while (!user.isCreative() && user.getItemInHand(hand).getDamageValue() != 0
                    && user.getInventory().countItem(KnocksItems.EARTH_RUNE.get()) > 0) {
                removeAmmo(KnocksItems.EARTH_RUNE.get(), user);
    ...
    public QuakeBlast createArrow(Level level, ItemStack stack, LivingEntity shooter) {
        float j = EnchantmentHelper.getTagEnchantmentLevel(Enchantments.POWER_ARROWS, stack);
        QuakeBlast arrowentity = new QuakeBlast(level, shooter,
                (2.0F + (j * 2.0F)));
        return arrowentity;
    }
