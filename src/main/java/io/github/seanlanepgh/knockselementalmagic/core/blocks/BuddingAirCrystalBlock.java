...
public class BuddingAirCrystalBlock extends AmethystBlock {
   public static final int GROWTH_CHANCE = 5;
   private static final Direction[] DIRECTIONS = Direction.values();

   public BuddingAirCrystalBlock(BlockBehaviour.Properties p_152726_) {
      super(p_152726_);
   }
   ...
   public void randomTick(BlockState p_220898_, ServerLevel p_220899_, BlockPos p_220900_, RandomSource p_220901_) {
      if (p_220901_.nextInt(5) == 0) {
         ...
         if (canClusterGrowAtState(blockstate)) {
            block = KnocksBlocks.SMALL_AIR_CRYSTAL_BUD.get();
         } else if (blockstate.is(KnocksBlocks.SMALL_AIR_CRYSTAL_BUD.get()) && blockstate.getValue(AmethystClusterBlock.FACING) == direction) {
            block = KnocksBlocks.MEDIUM_AIR_CRYSTAL_BUD.get();
         } else if (blockstate.is(KnocksBlocks.MEDIUM_AIR_CRYSTAL_BUD.get()) && blockstate.getValue(AmethystClusterBlock.FACING) == direction) {
            block = KnocksBlocks.LARGE_AIR_CRYSTAL_BUD.get();
         } else if (blockstate.is(KnocksBlocks.LARGE_AIR_CRYSTAL_BUD.get()) && blockstate.getValue(AmethystClusterBlock.FACING) == direction) {
            block = KnocksBlocks.AIR_CRYSTAL_CLUSTER.get();
         }
