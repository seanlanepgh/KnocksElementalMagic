...
public class ApprenticeEarthStaff extends KnocksBaseItem {

    public ApprenticeEarthStaff(Properties props) {
        super(props);
    }
    @Override
    public void releaseUsing(ItemStack stack,Level Level, LivingEntity entityLiving, int timeLeft) {
        ...
        if (entityLiving instanceof Player) {
            ...
            if (!Level.isClientSide) {
                QuakeBolt abstractarrowentity = createArrow(Level, stack, playerentity);
    ...
    public static void reload(Player user, InteractionHand hand) {
        if (user.getItemInHand(hand).getItem() instanceof ApprenticeIceStaff) {
    ...
    public QuakeBolt createArrow(Level level, ItemStack stack, LivingEntity shooter) {
        float j = EnchantmentHelper.getTagEnchantmentLevel(Enchantments.POWER_ARROWS, stack);
        QuakeBolt arrowentity = new QuakeBolt(level, shooter,
                (2.0F + (j * 2.0F)));
        return arrowentity;
    }
