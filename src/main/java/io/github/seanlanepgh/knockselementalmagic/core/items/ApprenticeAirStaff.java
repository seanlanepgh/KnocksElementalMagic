...
public class ApprenticeAirStaff extends KnocksBaseItem {

    public ApprenticeAirStaff(Properties props) {
        super(props);
    }
    @Override
    public void releaseUsing(ItemStack stack,Level Level, LivingEntity entityLiving, int timeLeft) {
        ...
        if (entityLiving instanceof Player) {
            ...
            if (!Level.isClientSide) {
                TornadoBolt abstractarrowentity = createArrow(Level, stack, playerentity);
    ...
    public static void reload(Player user, InteractionHand hand) {
        if (user.getItemInHand(hand).getItem() instanceof ApprenticeAirStaff) {
    ...
    public TornadoBolt createArrow(Level level, ItemStack stack, LivingEntity shooter) {
        float j = EnchantmentHelper.getTagEnchantmentLevel(Enchantments.POWER_ARROWS, stack);
        TornadoBolt arrowentity = new TornadoBolt(level, shooter,
                (2.0F + (j * 2.0F)));
        return arrowentity;
    }
