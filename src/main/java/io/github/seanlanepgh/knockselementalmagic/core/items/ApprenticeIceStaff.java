...
import io.github.seanlanepgh.knockselementalmagic.entity.projectile.BlizzBolt;
...
public class ApprenticeIceStaff extends KnocksBaseItem {

    public ApprenticeIceStaff(Properties props) {
        super(props);
    }
    @Override
    public void releaseUsing(ItemStack stack,Level Level, LivingEntity entityLiving, int timeLeft) {
        ...
        if (entityLiving instanceof Player) {
            ...
            if (!Level.isClientSide) {
                BlizzBolt abstractarrowentity = createArrow(Level, stack, playerentity);
    ...
    public BlizzBolt createArrow(Level level, ItemStack stack, LivingEntity shooter) {
        float j = EnchantmentHelper.getTagEnchantmentLevel(Enchantments.POWER_ARROWS, stack);
        BlizzBolt arrowentity = new BlizzBolt(level, shooter,
                (2.0F + (j * 2.0F)));
        return arrowentity;
    }
