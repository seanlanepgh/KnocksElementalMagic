...
public class IceWizardEntity extends SpellcasterIllager {
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(4, new IceWizardEntity.IceWizardEntitySummonSpellGoal());
    ...
    class IceWizardEntityAttackSpellGoal extends SpellcasterIllager.SpellcasterUseSpellGoal {
        ...
        private void createSpellEntity(double p_32673_, double p_32674_, double p_32675_, double p_32676_, float p_32677_, int p_32678_) {
            ...
            if (flag) {
                IceWizardEntity.this.level.addFreshEntity(new EvokerFangs(IceWizardEntity.this.level, p_32673_, (double)blockpos.getY() + d0, p_32674_, p_32677_, p_32678_, IceWizardEntity.this));
            }
    ...
    class IceWizardEntitySummonSpellGoal extends SpellcasterIllager.SpellcasterUseSpellGoal {
        ...
        public boolean canUse() {
            if (!super.canUse()) {
                ...
                int i = IceWizardEntity.this.level.getNearbyEntities(Vex.class, this.vexCountTargeting, IceWizardEntity.this, IceWizardEntity.this.getBoundingBox().inflate(16.0D)).size();
                return IceWizardEntity.this.random.nextInt(8) + 1 > i;
            }
        }
        ...
        protected void performSpellCasting() {
            ServerLevel serverlevel = (ServerLevel)IceWizardEntity.this.level;

            for(int i = 0; i < 3; ++i) {
                BlockPos blockpos = IceWizardEntity.this.blockPosition().offset(-2 + IceWizardEntity.this.random.nextInt(5), 1, -2 + IceWizardEntity.this.random.nextInt(5));
                Vex vex = EntityType.VEX.create(IceWizardEntity.this.level);
                vex.moveTo(blockpos, 0.0F, 0.0F);
                vex.finalizeSpawn(serverlevel, IceWizardEntity.this.level.getCurrentDifficultyAt(blockpos), MobSpawnType.MOB_SUMMONED, (SpawnGroupData)null, (CompoundTag)null);
                vex.setOwner(IceWizardEntity.this);
                vex.setBoundOrigin(blockpos);
                vex.setLimitedLife(20 * (30 + IceWizardEntity.this.random.nextInt(90)));
