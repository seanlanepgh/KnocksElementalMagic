...
public class QuakeKnightEntity extends KnocksEntity implements IAnimatable, IAnimationTickable, Enemy {
    private float allowedHeightOffset = 0.5F;
    private int nextHeightOffsetChangeTick;
    private AnimationFactory factory = new AnimationFactory(this);

    public QuakeKnightEntity(EntityType<? extends KnocksEntity> entityType, Level level) {
    ...
    protected void registerGoals() {
        this.goalSelector.addGoal(4, new RangedAttackGoal(this,
                new QuakeKnightEntity.QuakeBoltAttack(this).setProjectileOriginOffset(0.8, 0.5, 0.8)
                        .setDamage(3.0F),1.0F));
    ...
    public void registerControllers(AnimationData data) {
        data.addAnimationController(new AnimationController<QuakeKnightEntity>(this, "controller", 0, this::predicate));
    }

    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        if (event.isMoving()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.quake_knight.walk", true));
            return PlayState.CONTINUE;
        }
        event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.quake_knight.idle", true));
        return PlayState.CONTINUE;

    }
    private static final EntityDataAccessor<Byte> DATA_FLAGS_ID = SynchedEntityData.defineId(QuakeKnightEntity.class, EntityDataSerializers.BYTE);

    public class QuakeBoltAttack extends AbstractRangedAttack {

        public QuakeBoltAttack(QuakeKnightEntity parentEntity, double xOffSetModifier, double entityHeightFraction,
                               double zOffSetModifier, float damage) {
            super(parentEntity, xOffSetModifier, entityHeightFraction, zOffSetModifier, damage);
        }

        public QuakeBoltAttack(QuakeKnightEntity parentEntity) {
            super(parentEntity);
        }
