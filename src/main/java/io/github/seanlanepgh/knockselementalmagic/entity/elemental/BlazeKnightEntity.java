...
public class BlazeKnightEntity extends KnocksEntity implements IAnimatable, IAnimationTickable, Enemy {
    private float allowedHeightOffset = 0.5F;
    private int nextHeightOffsetChangeTick;
    private AnimationFactory factory = new AnimationFactory(this);

    public BlazeKnightEntity(EntityType<? extends KnocksEntity> entityType, Level level) {
    ...
    protected void registerGoals() {
        this.goalSelector.addGoal(4, new RangedAttackGoal(this,
                new BlazeKnightEntity.BlazeKnightBoltAttack(this).setProjectileOriginOffset(0.8, 0.5, 0.8)
                        .setDamage(3.0F),1.0F));
    ...
    public void registerControllers(AnimationData data) {
        data.addAnimationController(new AnimationController<BlazeKnightEntity>(this, "controller", 0, this::predicate));
    }

    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        if (event.isMoving()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.blaze_knight.walk", true));
            return PlayState.CONTINUE;
        }
        event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.blaze_knight.idle", true));
        return PlayState.CONTINUE;
    }
    ...
    public class BlazeKnightBoltAttack extends AbstractRangedAttack {
        ...
        public BlazeKnightBoltAttack(BlazeKnightEntity parentEntity, double xOffSetModifier, double entityHeightFraction,
                               double zOffSetModifier, float damage) {
            super(parentEntity, xOffSetModifier, entityHeightFraction, zOffSetModifier, damage);
        }

        public BlazeKnightBoltAttack(BlazeKnightEntity parentEntity) {
            super(parentEntity);
        }
